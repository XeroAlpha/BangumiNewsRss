...
async function main() { 
    ...
    if (config.agent) {
        if (config.agent.type == "http") {
            requestAgent = { // 别听d.ts里瞎说，ca、key、cert都支持string
    ...
    credential = await api.login(config.site, config.user, config.pass, { agent: requestAgent });
        ...
        createAgentIfAbsent: async function(options) {
            ...
            if (!agent) {
                try {
                    ...
                    console.time("Create Agent");
                    agent = await api.Agent.create(credential, options);
                    console.timeEnd("Create Agent");

...
const agentTemplates = {
    ...
    extractorGenerator: function(options, definition) {
        ...
        Agent.receive = function() {
            ...
            events.map(e => {
                ...
                let extractor = generator(e.payload.url, e.payload) || { bodySelector: "body", bodyXPath: "请在浏览器里打开" };
                return {
                    ...e.payload,
                    ...extractor
                };
    ...
    merge: function(options, definition) {
        ...
        let agentOptions = {
            instructions: {
                link: options.description ? "{{url}}" : `${options.url}#{{id}}`,
                title: "{{title}}",
                date: "{{date}}",
                description: "{{body}}",
                guid: `${options.guidPrefix}#{{id}}`,
...
async function main() { 
    ...
    let credential = await api.login(config.site, config.user, config.pass);
    let data = {
        agents: await api.Agent.list(credential, { sort: "created_at.desc" }),
        scenarios: await api.Scenario.list(credential),
        sources: {}
    };
    let host = {
        definition: {},
        ...
        updateAgent: async function(agent, patch) {
            ...
            if (index >= 0) {
                ...
                console.log("Agent Updated: " + newAgent.name);
                return newAgent;
            }
            return null;
        },

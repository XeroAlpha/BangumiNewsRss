...
function transformToPhantomJsCloudUrlIfNeeded(urlOrOptions) {
    if (typeof urlOrOptions == "object") {
        return `https://PhantomJsCloud.com/api/browser/v2/{% credential ${urlOrOptions.apiKey||"PhantomJsCloudKey"} %}/?request=${encodeURIComponent(JSON.stringify(urlOrOptions))}`;
    } else {
        return urlOrOptions;
    }
}
...
const agentTemplates = {
    ...
    index: function(options) {
        ...
        let agentOptions = {
            ...
            url: transformToPhantomJsCloudUrlIfNeeded(options.url),
...
async function main() { 
    ...
    let host = {
        ...
        updateAgent: async function(agent, patch) {
            ...
            if (index >= 0) {
                try {
                    ...
                    console.error("Updating Agent Failed: ", options)
                    throw err;
                }
            }
            return null;
        ...
        updateOrCreatePipe: async function(options) {
            ...
            for (i = 0; i < pipe.length; i++) {
                ...
                let oldAgent = this.findAgentByName(currentAgentOptions.name), newAgent;
                if (i > 0) {
                ...
                newAgent = await this.updateOrCreateAgent(currentAgentOptions, wip);
        ...
        addSource: async function(options) {
            ...
            if (this.definition.dryRun || options.dryRun) {
                ...
                let updateCount = await this.updateOrCreatePipe({
                    pipe: agentPipe,
    ...
    let boundHost = {
        ...
        skipSource: host.skipSource.bind(host),
        mockSeason: host.mockSeason.bind(host),
        defineShortcut: host.defineShortcut.bind(host),
        offline: true, // 仅用于引用
        dryRun: true,
        wip: true,
        hideFromRss: true
